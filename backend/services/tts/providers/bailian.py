    ...
    *,
    ...
    try:
        ...
        for chunk in r.iter_content(chunk_size=4096):
            if cancel_event.is_set():
                logger.info(f"[{request_id}] bailian_http_tts_cancelled")
                break
            if chunk:
                yield chunk
    finally:
...
def stream_bailian_tts_dashscope(
    *,
    text: str,
    request_id: str,
    config: dict,
    ...
    synthesizer_callback = Callback(logger)
    ...
    try:
        ...
        def _try_parse_wav_header(buf: bytes):
            ...
            while offset + 8 <= len(buf):
                ...
                if chunk_id == b"fmt " and chunk_size >= 16:
                    audio_format_val = u16(offset)
                    channels = u16(offset + 2)
                    sample_rate_val = u32(offset + 4)
        ...
        while True:
            if cancel_event.is_set():
                canceled = True
                stop_event.set()
                logger.info(f"[{request_id}] dashscope_tts_cancelled")
            ...
            try:
                ...
                if first_chunk and first_chunk_timeout_s > 0 and (time.perf_counter() - t_call) >= first_chunk_timeout_s:
                    logger.error(f"[{request_id}] dashscope_tts_first_chunk_timeout timeout_s={first_chunk_timeout_s} (canceling)")
                    canceled = True
                    with contextlib.suppress(Exception):
                        speech_synthesizer.streaming_cancel()
                    break
            ...
            if first_chunk:
                ...
                logger.info(f"[{request_id}] dashscope_tts_first_chunk dt={time.perf_counter() - t_call:.3f}s bytes={len(item)} riff={is_riff}")
                if not is_riff:
