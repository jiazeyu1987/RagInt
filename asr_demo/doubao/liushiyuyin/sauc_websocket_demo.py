...
class Config:
    def __init__(self):
        # 濉叆鎺у埗鍙拌幏鍙栫殑app id鍜宎ccess token
        self.auth = {
            "app_key": os.getenv("DOUBAO_APP_KEY", DEFAULT_APP_KEY),
            "access_key": os.getenv("DOUBAO_ACCESS_KEY", DEFAULT_ACCESS_KEY)
        }
        self.resource_id = os.getenv("DOUBAO_RESOURCE_ID", DEFAULT_RESOURCE_ID)
...
class RequestBuilder:
    ...
    @staticmethod
    def new_full_client_request(seq: int) -> bytes:  # 娣诲姞seq鍙傛暟
        ...
        request.extend(struct.pack('>i', seq))  # 浣跨敤浼犲叆鐨剆eq
...
class ResponseParser:
    ...
    def parse_response(msg: bytes) -> AsrResponse:
        ...
        # 瑙ｆ瀽message_type_specific_flags
        ...
        # 瑙ｅ帇缂?        if message_compression == CompressionType.GZIP:
            try:
...
class AsrWsClient:
    def __init__(self, url: str, segment_duration: int = 200):
        ...
        self.conn = None
        self.session = None  # 娣诲姞session寮曠敤
    ...
    async def create_connection(self) -> None:
        ...
        try:
            self.conn = await self.session.ws_connect(  # 浣跨敤self.session
                self.url,
                headers=headers
            )
    ...
    async def send_messages(self, segment_size: int, content: bytes) -> AsyncGenerator[None, None]:
        ...
        for i, segment in enumerate(audio_segments):
            ...
            await asyncio.sleep(self.segment_duration / 1000) # 閫愪釜鍙戦€侊紝闂撮殧鏃堕棿妯℃嫙瀹炴椂娴?            # 璁╁嚭鎺у埗鏉冿紝鍏佽鎺ュ彈娑堟伅
            yield
...
async def main() -> int:
    ...
    parser.add_argument("--app-key", type=str, default=os.getenv("DOUBAO_APP_KEY", DEFAULT_APP_KEY), help="X-Api-App-Key")
    parser.add_argument("--access-key", type=str, default=os.getenv("DOUBAO_ACCESS_KEY", DEFAULT_ACCESS_KEY), help="X-Api-Access-Key")
    parser.add_argument("--resource-id", type=str, default=os.getenv("DOUBAO_RESOURCE_ID", DEFAULT_RESOURCE_ID), help="X-Api-Resource-Id")
    ...
    if not args.app_key or not args.access_key:
        raise ValueError("Missing credentials. Set --app-key/--access-key or DOUBAO_APP_KEY/DOUBAO_ACCESS_KEY.")
    config.auth["app_key"] = args.app_key
    config.auth["access_key"] = args.access_key
...
if __name__ == "__main__":
    ...
    # 鐢ㄦ硶锛?    # python3 sauc_websocket_demo.py --file /Users/bytedance/code/python/eng_ddc_itn.wav
