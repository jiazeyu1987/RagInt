...
> 说明：本文严格从 `doc/robot/03-software-requirements.md` 拆解而来，按 FR/NFR 编号逐条落到可实现的设计。本文描述软件架构、数据流、状态机与接口（对接 RAG/ASR/TTS/导航），并给出异常与降级策略。若系统侧接口/点位由第三方提供，以 `doc/robot/04-detailed-design.md` 的“对接能力语义”为准。
...
> 实现状态：[已实现]（连续讲解 prompt/分段输出/站点时长与目标字数规划；前端具备 Tour 流水线与预取）
> 代码证据：`backend/orchestrators/conversation_orchestrator.py`、`backend/services/tour_planner.py`、`fronted/src/managers/TourPipelineManager.js`
...
> 实现状态：[已实现]（request_id 级取消；前端 interrupt 停止播放/停止 SSE/取消后端；后端支持 cancel_active）
> 代码证据：`backend/services/request_registry.py`、`backend/infra/cancellation.py`、`backend/app.py`（`/api/cancel`、`/api/ask`）、`fronted/src/managers/AskWorkflowManager.js`
...
> 实现状态：[已实现]（流式 TTS + 分段队列播放 + 中断清空；包含白噪/异常音频探测与回退播放链路）
> 代码证据：`backend/app.py`（`/api/text_to_speech_stream`）、`backend/services/tts_service.py`、`fronted/src/managers/TtsQueueManager.js`、`fronted/src/audio/ttsAudio.js`
...
> 实现状态：[已实现]（覆盖 SD-6.1/6.2：request_id/client_id/stop/action 记录，提交→首字→首段→TTS首包→播报结束时间线，nav_* 事件，调试面板展示与 NDJSON 导出）
> 代码证据：`backend/app.py`（`/api/status`、`/api/events`、`/api/client_events`、`/api/ask`、`/api/text_to_speech_stream`）、`backend/infra/event_store.py`、`backend/services/request_registry.py`、`fronted/src/api/backendClient.js`、`fronted/src/managers/AskWorkflowManager.js`、`fronted/src/managers/TtsQueueManager.js`、`fronted/src/hooks/useBackendStatus.js`、`fronted/src/hooks/useBackendEvents.js`、`fronted/src/components/DebugPanel.js`
...
> 实现状态：[部分实现]（RAG/ASR/TTS 均有不同程度的 fallback；但“按站点本地脚本讲解/离线兜底”的完整策略未在仓库内落地）
> 代码证据：`backend/orchestrators/conversation_orchestrator.py`（RAGFlow 不可用 fallback）、`backend/services/asr_service.py`（多 provider fallback）、`backend/services/tts_service.py`（provider 与播放回退）、`fronted/src/audio/ttsAudio.js`
...
- 讲解降级：使用本地脚本（按站点准备简短讲解文本），至少能显示文本并可播报（若 TTS 可用）
...
> 实现状态：[部分实现]（有 rate limit、关键时延埋点与状态查询；未见系统化压测/长期稳定性守护进程/自动化巡检脚本落地）
> 代码证据：`backend/services/request_registry.py`（rate limit）、`backend/app.py`（timings + `/api/status`）、`fronted/src/hooks/useBackendStatus.js`
...
| 角色 | 主要职责 | 建议投入（人月） |
|---|---|---:|
| 后端/编排工程师 | SSE/取消、编排与降级、对接适配、日志指标 | 3.0 |
| 前端/控制端工程师 | 控制台、录音播放、队列与中断、状态面板 | 2.0 |
| 数据/知识库工程师 | 清洗入库、元数据、版本与回滚、抽检流程 | 2.0 |
| 测试/交付 | 用例与压测、稳定性、文档与交付收口 | 1.0 |
|  |  | **8.0** |
